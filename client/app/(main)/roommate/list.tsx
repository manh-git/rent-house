...
export default function RoommateIndex() {
  ...
  const fetchData = async (params: any = {}) => {
    setLoading(true);
    try {
      const query = { index: 1, count: 20, ...params };
      const result = await getListRoommateAPI(query);
  ...
  const performDelete = async () => {
    try {
      ...
      setData(prevData => prevData.filter((item: any) => item.request_id !== id));
    } catch (error) {
      ...
      <View style={styles.topbar}>
        ...
        >
          <Ionicons
            ...
            size={18}
            color={activeFilterCount > 0 ? '#534AB7' : '#5F5E5A'}
          />
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {isEditing ? 'Chỉnh sửa bài đăng' : 'Đăng tìm bạn cùng phòng'}
              </Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.pickerWrap}>
                <Picker
                  ...
                  onValueChange={(val) => setFilterParams({ ...filterParams, ward_id: val })}
                  style={styles.picker}
                >
                  <Picker.Item label="Tất cả khu vực" value="" />
                  {wards.map((w: any) => (
                    <Picker.Item key={w.ward_id} label={w.ward_name} value={String(w.ward_id)} />
                  ))}
                </Picker>
              ...
              <View style={styles.pickerWrap}>
                <Picker
                  ...
                  onValueChange={(val) => setFilterParams({ ...filterParams, preferred_gender: val })}
                  style={styles.picker}
                >
              ...
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    placeholderTextColor="#B4B2A9"
                    keyboardType="numeric"
                    value={filterParams.min_age}
                    onChangeText={(val) => setFilterParams({ ...filterParams, min_age: val })}
                  />
                </View>
                <View style={{ width: 12 }} />
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    placeholderTextColor="#B4B2A9"
                    keyboardType="numeric"
                    value={filterParams.max_age}
                    onChangeText={(val) => setFilterParams({ ...filterParams, max_age: val })}
                  />
                </View>
              </View>
